...
int OnInit()
{
   g_symbolToUse = StringTrim(InpSymbol);
   if(g_symbolToUse == "")
      g_symbolToUse = _Symbol;

   if(!SymbolSelect(g_symbolToUse, true))
   {
      PrintFormat("Failed to select symbol '%s'.", g_symbolToUse);
      return(INIT_FAILED);
   }

   g_atrHandle = iATR(g_symbolToUse, InpTimeframe, InpATRPeriod);
   if(g_atrHandle == INVALID_HANDLE)
   {
      PrintFormat("Failed to create ATR handle for %s on timeframe %d. Error: %d",
                  g_symbolToUse, InpTimeframe, GetLastError());
      return(INIT_FAILED);
   }
   ...
   PrintFormat("ATR alert initialized for %s, TF=%s, period=%d, trigger=%.3f",
               g_symbolToUse, EnumToString(InpTimeframe), InpATRPeriod, InpATRTriggerLevel);
...
void OnTimer()
{
   ...
   if(atrValues[0] > InpATRTriggerLevel && g_timerActive)
      ResetTimerState();

   UpdateAtrDisplay(g_symbolToUse, atrValues[0]);
}
...
void OnTick()
{
   ...
   // Keep timer strictly tied to below-trigger state.
   if(currentATR > InpATRTriggerLevel && g_timerActive)
      ResetTimerState();

   if(InpDisplayAtrOnChart)
      UpdateAtrDisplay(g_symbolToUse, currentATR);

   datetime currentBarTime = iTime(g_symbolToUse, InpTimeframe, 0);
   if(currentBarTime == 0)
      return;
   ...
   if(triggerNow)
      SendAtrAlert(g_symbolToUse, currentATR, currentBarTime);
...
void UpdateAtrDisplay(string symbolName, double atrValue)
{
   ...
   if(g_timerActive && g_timerStartTime > 0 && atrValue <= InpATRTriggerLevel)
   {
